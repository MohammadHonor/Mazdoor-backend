...
from app.schemas.animalSchema import ErrorResponse
...
# @router.get("/animal/{name}",responses={400 : { "model": "hi"}})
# async def show_animal_list(name:str,session:Annotated[AsyncSession , Depends(get_session)]):
#     try:
#         repo = AnimalRepo(session=session)
#         result = await repo.display_by_name(name=name) 
#         if not result :
...
async def show_animal_list(name: str, session:Annotated[AsyncSession,Depends(get_session)]):
    ...
    if name == "":
        return JSONResponse(status_code=400,content="name is null")
    try:
        repo = AnimalRepo(session=session)
        result = await repo.display_by_name(name=name)
